...
def run_suite(args: argparse.Namespace) -> List[Dict[str, object]]:
    ...
    if run_natural:
        heldout = load_heldout_cases(
            ...
            val_fraction=args.sft_val_fraction,
            split_seed=args.sft_split_seed,
        )

        if heldout:
            print(
                f"Loaded {len(heldout)} real held-out SFT queries from {args.sft_dir}.\n",
                flush=True,
            )
            natural_test_cases.extend(heldout)
        else:
            print(
                "WARNING: could not reconstruct held-out SFT queries; "
    ...
    if run_structured:
        ...
        for case in struct_cases:
            ...
            available = config.max_seq_len - len(prompt_ids)
            if available <= 0:
                raise RuntimeError(
    ...
    json_path = output_dir / "inference_results_v2.json"
    txt_path = output_dir / "inference_results_v2.txt"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run a 20-query heldout/generalization/control inference diagnostic against final UO-Mind."
    )
